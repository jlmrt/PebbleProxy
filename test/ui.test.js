...
test('device and Needle diagnostics UI use the nested API contracts', () => {
  ...
  assert.match(html, /<span>Ring Button<\/span><select name="indexTrigger">/);
  assert.match(html, /value="single-click-hold">Hold &amp; Talk/);
  assert.match(html, /value="double-click-hold">Double click &amp; hold/);
  assert.match(html, /value="all">Both/);
  assert.match(html, /value="custom">Custom/);
  assert.match(html, /id="connection-name-field"/);
  assert.match(html, /name="mcpTopic"/);
  assert.match(html, /value="notes">Notes/);
  assert.match(html, /value="reminders">Reminders/);
  assert.match(html, /value="calendar" disabled/);
  assert.match(html, /value="messaging" disabled/);
  assert.doesNotMatch(html, /Index gesture/);
  ...
  assert.match(script, /body\.indexTrigger = triggerChoice === "custom" \? "all" : triggerChoice/);
  assert.match(script, /body\.mcpTopic = plainText\(data\.get\("mcpTopic"\), "notes"\)/);
  assert.match(script, /isCustomWebhook = isWebhook && triggerChoice === "custom"/);
  assert.match(script, /form\.elements\.label\.required = isCustomWebhook/);
  assert.match(script, /Pebble Index \$\{mcpTopicLabel\(mcpTopic\)\} MCP server/);
